...
package object perspective extends LowPriorityPackage1 {
  ...
  type #~>:[T, F[_]] = Const[T]#λ ~>: F
  type #~>#:[T, R]   = Const[T]#λ ~>: Const[R]#λ

  type Tuple2K[F[_], G[_]]                   = { type λ[A] = (F[A], G[A]) }
  type Tuple3K[F[_], G[_], H[_]]             = { type λ[A] = (F[A], G[A], H[A]) }
  type Tuple4K[F[_], G[_], H[_], I[_]]       = { type λ[A] = (F[A], G[A], H[A], I[A]) }
  type Tuple5K[F[_], G[_], H[_], I[_], J[_]] = { type λ[A] = (F[A], G[A], H[A], I[A], J[A]) }
  ...
  with TraverseK[IdF#λ] with DistributiveK[IdF#λ] {
    ...
    override def traverseK[G[_]: Applicative, A[_], B[_], C](fa: A[C])(f: A ~>: λ[D => G[B[D]]]): G[B[C]] = f(fa)

    override def foldLeftK[A[_], B, C](fa: A[C], b: B)(f: B => A ~>#: B): B = f(b)(fa)

    override def map2K[A[_], B[_], Z[_], C](fa: A[C], fb: B[C])(f: Tuple2K[A, B]#λ ~>: Z): Z[C] = f(fa, fb)
  ...
  trait LowPriorityPackage1 {
    ...
    implicit def idInstanceC[T]: ApplicativeKC[IdFC[T]#λ] with TraverseKC[IdFC[T]#λ] with DistributiveKC[IdFC[T]#λ] =
      new ApplicativeKC[IdFC[T]#λ] with TraverseKC[IdFC[T]#λ] with DistributiveKC[IdFC[T]#λ] {
        ...
        override def traverseK[G[_]: Applicative, A[_], B[_], C](fa: A[T])(f: A ~>: λ[D => G[B[D]]]): G[B[T]] = f(fa)

        override def foldLeftK[A[_], B, C](fa: A[T], b: B)(f: B => A ~>#: B): B = f(b)(fa)

        override def map2K[A[_], B[_], Z[_], C](fa: A[T], fb: B[T])(f: Tuple2K[A, B]#λ ~>: Z): Z[T] = f(fa, fb)
